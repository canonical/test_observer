...
export default function UserAvatar({ 
  ...
  const ratioCompleted = useMemo(() => {
    if (allEnvironmentReviewsCount === 0) return 0
  ...
  const progressColor = useMemo(() => {
    ...
    return adjustLightness(avatarColor, -0.2)
  }, [avatarColor])

  const circumference = 2 * Math.PI * 19.5
  const progressOffset = circumference * (1 - ratioCompleted)
  ...
  const tooltipMessage = useMemo(() => {
    ...
    return result
  }, [user, isEmpty, ratioCompleted, allEnvironmentReviewsCount, completedEnvironmentReviewsCount])

  return (
    <div className="user-avatar" title={tooltipMessage}>
      <svg className="avatar-svg" width="43" height="43" viewBox="0 0 43 43">
      ...
      {/* Initials text */}
      {!isEmpty && <span className="initials">{initials}</span>}
    </div>
  )
}
