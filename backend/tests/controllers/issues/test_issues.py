...
def test_get_all_affected_runs_count(test_client: TestClient, generator: DataGenerator):
    ...
    assert issues_by_id[issue.id]["affected_runs_count"] == 2
    assert issues_by_id[unrelated_issue.id]["affected_runs_count"] == 0
