...
"""Move solution-specific bundled build fields into artefact.attributes

Backfills the generic ``attributes`` field from the legacy solution-specific
data, swaps the ``unique_solution`` index to ``(name, version)`` and drops the
now-unused ``artefact.bundled_builds_hash`` column and the
``artefact_bundled_builds_association`` table.

Revision ID: f3a9c7d21b84
Revises: a2cce585712b
Create Date: 2026-08-31 22:46:00.000000+00:00
...
revision = "f3a9c7d21b84"
down_revision = "a2cce585712b"
branch_labels = None
depends_on = None
